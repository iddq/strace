...
# define PRINT_FIELD_FRACT(prefix_, where_, field_)			\
	STRACE_PRINTF("%s%s=%u/%u", (prefix_), #field_,			\
		      (where_).field_.numerator, (where_).field_.denominator)
...
print_pixelformat(uint32_t fourcc, const struct xlat *xlat)
{
	...
	unsigned char a[] = {
		...
		(unsigned char) (fourcc >> 24) & 0x7f,
	};
	unsigned int i;

	tprints(fourcc & 0x80000000 ? "v4l2_fourcc_be(" : "v4l2_fourcc(");
...
print_v4l2_fmtdesc(struct tcb *const tcp, const kernel_ulong_t arg)
{
	...
	if (!syserror(tcp) && !umove(tcp, arg, &f)) {
		...
		const struct xlat *fmt_xlat = v4l2_pix_fmts;
		 ...
		 const kernel_ulong_t arg)
{
	...
	PRINT_FIELD_CSTRING("", a, name);
	PRINT_FIELD_FLAGS(", ", a, capability, v4l2_audio_capabilities,
			  "V4L2_AUDCAP_???");
	PRINT_FIELD_FLAGS(", ", a, mode, v4l2_audio_modes, "V4L2_AUDMODE_???");
	if (!IS_ARRAY_ZERO(a.reserved)) {
